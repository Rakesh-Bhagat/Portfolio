    ...
    <div className="flex flex-col border border-neutral-300 p-5 rounded-lg shadow-lg hover:shadow-2xl">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2 ">
        {stack.map((item, index) => (
          <div className="bg-blue-50 rounded-full px-3 text-center font-semibold py-1 text-neutral-700 text-xs" key={index}>{item}</div>
        ))}
      </div>
      ...
      {github && <Link target="_blank" href={github}><p className="flex gap-2 bg-gray-900 text-neutral-200 px-4 rounded-lg text-sm py-1"><CodeXml width={20} />github</p></Link>}
      </div>
